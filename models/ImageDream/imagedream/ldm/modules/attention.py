...
class MemoryEfficientCrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        # 2. BLACKWELL OPTIMIZED SHAPING
        # SDPA prefers (B, H, N, D)
        def prepare(t):
            return rearrange(t, 'b n (h d) -> b h n d', h=h).contiguous()

        q, k, v = prepare(q), prepare(k), prepare(v)

        # 3. NATIVE SDPA (FlashAttention-3 on 5090)
        out = torch.nn.functional.scaled_dot_product_attention(q,
                                                               k,
                                                               v,
                                                               dropout_p=0.0)

        # 4. RESTORE IP-ADAPTER ATTENTION PASS
        if has_ip:
            k_ip = rearrange(k_ip, 'b n (h d) -> b h n d', h=h).contiguous()
            v_ip = rearrange(v_ip, 'b n (h d) -> b h n d', h=h).contiguous()
            out_ip = torch.nn.functional.scaled_dot_product_attention(
                q, k_ip, v_ip, dropout_p=0.0)
...
class SpatialTransformer3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.transformer_blocks = nn.ModuleList([
            BasicTransformerBlock3D(inner_dim,
                                    n_heads,
                                    d_head,
                                    dropout=dropout,
                                    context_dim=context_dim[d],
