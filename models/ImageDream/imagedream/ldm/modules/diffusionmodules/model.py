...
class MemoryEfficientAttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # 1. Use existing separate layers (matches your model weights)
        q = self.q(h_)
        k = self.k(h_)
        v = self.v(h_)

        # 2. Prepare shapes for Attention
        # Current shape: (Batch, Channels, Height, Width)
        B, C, H, W = q.shape

        # Flatten spatial dimensions to Sequence
        # New shape: (Batch, H*W, Channels) -> (Batch, Seq, Dim)
        # We use a manual reshape to avoid needing 'einops' here if not imported
        q = q.permute(0, 2, 3, 1).reshape(B, H * W, C)
        k = k.permute(0, 2, 3, 1).reshape(B, H * W, C)
        v = v.permute(0, 2, 3, 1).reshape(B, H * W, C)

        # ============================================================
        # RTX 5090 FIX: BFLOAT16 + PURE PYTORCH
        # ============================================================
        # 4. Run Standard PyTorch Attention (SDPA)
        # We bypass xformers entirely.
        # SDPA naturally handles (Batch, Seq, Dim) inputs (3D tensors).
        out = torch.nn.functional.scaled_dot_product_attention(
            q, k, v, attn_mask=None, dropout_p=0.0,
            is_causal=False).contiguous()

        # ============================================================

        # 5. Reshape back to Image format
        # (Batch, Seq, Dim) -> (Batch, Height, Width, Channels)
