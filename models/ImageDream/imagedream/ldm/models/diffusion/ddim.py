...
class DDIMSampler(object):
    ...
    def make_schedule(self,
        ...
        self.register_buffer("sqrt_alphas_cumprod",
                             to_torch(np.sqrt(alphas_cumprod.float().cpu())))
        self.register_buffer(
            "sqrt_one_minus_alphas_cumprod",
            to_torch(np.sqrt(1.0 - alphas_cumprod.float().cpu())),
        )
        safe_alpha = np.clip(1.0 - alphas_cumprod.float().cpu(), 1e-20, 1.0)
        self.register_buffer("log_one_minus_alphas_cumprod",
                             to_torch(np.log(safe_alpha)))
        self.register_buffer(
            "sqrt_recip_alphas_cumprod",
            to_torch(np.sqrt(1.0 / alphas_cumprod.float().cpu())))
        self.register_buffer(
            "sqrt_recipm1_alphas_cumprod",
            to_torch(np.sqrt(1.0 / alphas_cumprod.float().cpu() - 1)),
        )

        # ddim sampling parameters
        ddim_sigmas, ddim_alphas, ddim_alphas_prev = make_ddim_sampling_parameters(
            alphacums=alphas_cumprod.float().cpu(),
    ...
    def sample_ddpm(
        ...
        **kwargs,
        ...
        # schedule is created beforehand
        #self.make_schedule(ddim_num_steps=S, ddim_eta=eta, verbose=verbose)
        # sampling
        C, H, W = shape
        size = (batch_size, C, H, W)
        samples, intermediates = self.ddpm_sampling(
            conditioning,
            size,
            callback=callback,
    ...
    def ddpm_sampling(
        ...
        **kwargs,
        ...
        if x_T is None:
            ...
            print("WATCHOUT: using random noise as x_T in DDPM sampling")
        else:
            img = x_T
        if timesteps is None:
            timesteps = (self.ddpm_num_timesteps
                         if ddim_use_original_steps else self.ddim_timesteps)
        ...
        total_steps = timesteps if ddim_use_original_steps else timesteps.shape[
            0]
        iterator = tqdm(time_range, desc="DDPM-Inv Sampler", total=total_steps)
    ...
    def p_sample_ddpm(
        ...
        **kwargs,
        ...
        current__sigma = self.ddim_sigmas[index]
        sigma_t = torch.full((b, 1, 1, 1), current__sigma, device=device)
